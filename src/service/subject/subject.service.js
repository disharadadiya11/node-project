...
module.exports = class SubjectService {
  ...
  async updateSubject(user, params, Body) {
    try {
      ...
      let subject = await this.subjectModel.findByIdAndUpdate(params.id, Body, {
        new: true,
      });
      if (!subject)
        throw new ErrorExceptionWithResponse(
  ...
  async deleteSubject(user, Body) {
    try {
      let subject = await this.subjectModel.updateMany(
        { _id: { $in: Body.ids } },
  ...
  async getAllSubject(user, Query) {
    try {
      ...
      const specificUserMatch =
        Query && "user" in Query
          ? [
              {
                $match: {
                  createdBy: new mongoose.Types.ObjectId(Query.user),
                },
              },
            ]
          : [];
