...
module.exports = class UserService {
  constructor() {
    ...
    this.cloudnaryService = new CloudnaryService();
  }
  ...
  async register(Body, file) {
    try {
      ...
      await this.emailService.sendMail(
        user?.email,
        emailConstant.WELCOME,
        verfiyEmailRegistarionTemplate(user)
      );
      console.log(
        "Email sent successfully___________________________________________________"
      );
      return successResponse(StatusCodes.OK, false, MSG.REGISTER_SUCCESS, user);
    } catch (error) {
      throw new ErrorExceptionWithResponse(
    ...
    try {
      ...
      if (user.isEmailVerify == true)
        throw new ErrorExceptionWithResponse(
          StatusCodes.BAD_REQUEST,
          true,
          MSG.EMAIL_ALREADY_VERIFY
        );

      user.isEmailVerify = true;
      await user.save();
      return successResponse(StatusCodes.OK, false, MSG.EMAIL_VERIFY_SUCCESS);
    } catch (error) {
      throw new ErrorExceptionWithResponse(
        error.status || StatusCodes.BAD_REQUEST,
        true,
  ...
  //                   throw new ErrorExceptionWithResponse(StatusCodes.BAD_REQUEST, true, MSG.PLEASE_REGISTER);

  //             if (!await (passwordDecrypt(Body.password, user.password)))
  //                   throw new ErrorExceptionWithResponse(StatusCodes.BAD_REQUEST, true, MSG.WRONG_CREDENTIAL);

  //             if (user.isEmailVerify == false)
  //                   throw new ErrorExceptionWithResponse(StatusCodes.BAD_REQUEST, true, MSG.PLEASE_VERIFY_EMAIL);
  ...
  //                   password: undefined
  //             };
  //             return successResponse(StatusCodes.OK, false, MSG.LOGIN_SUCCESS, user);
  //       }
  //       catch (error) {
  //             throw new ErrorExceptionWithResponse(
  //                   error.status || StatusCodes.BAD_REQUEST,
  ...
  async login(session, Body) {
    try {
      let user = await this.userModel
        ...
        .select("+password +isEmailVerify +role");
  ...
  async forgetPassword(Body) {
    try {
      ...
      await this.emailService.sendMail(
        findUser?.email,
        emailConstant.WELCOME,
        forgetPasswordTemplate(findUser)
      );

      return successResponse(StatusCodes.OK, false, MSG.EMAIL_SEND_SUCCESS);
    } catch (error) {
      throw new ErrorExceptionWithResponse(
        error.status || StatusCodes.BAD_REQUEST,
        true,
    ...
    try {
      ...
      if (Body.newPassword != Body.confirmPassword)
        throw new ErrorExceptionWithResponse(
          StatusCodes.BAD_REQUEST,
          true,
          MSG.PASSSWORD_NOT_MATCH
        );

      user.password = await passwordEncrypt(Body.newPassword);
      await user.save();

      return successResponse(StatusCodes.OK, false, MSG.PASSWORD_RESET_SUCCESS);
    } catch (error) {
      throw new ErrorExceptionWithResponse(
        error.status || StatusCodes.BAD_REQUEST,
        true,
  ...
  async updateProfile(user, Body, file) {
    try {
      ...
      let findUser = await this.userModel.findOneAndUpdate(user._id, Body, {
        new: true,
      });
      if (!findUser)
        throw new ErrorExceptionWithResponse(
  ...
  async removeProfile(user) {
    try {
      let findUser = await this.userModel.findByIdAndDelete(user._id);
      if (!findUser)
        throw new ErrorExceptionWithResponse(
          StatusCodes.BAD_REQUEST,
          true,
      ...
      let publicId = await extractPublicIdFromImageUrl(findUser?.image);
      await this.cloudnaryService.deleteImageFromFolder(publicId, "user");
  ...
  //             return successResponse(StatusCodes.OK, false, MSG.LOGOUT_SUCCESS);
  //       }
  //       catch (error) {
  //             throw new ErrorExceptionWithResponse(
  //                   error.status || StatusCodes.BAD_REQUEST,
