...
const { successResponse, validateCsvFile } = require("../utils/common.utils");
...
module.exports = class CourseService {
  ...
  async createCourse(user, Body, files) {
    try {
      // If images are uploaded in our database
      ...
      return successResponse(StatusCodes.OK, false, MSG.CREATE_SUCCESS, course);
    } catch (error) {
  ...
  async updateCourse(user, params, Body, files) {
    try {
      ...
      return successResponse(StatusCodes.OK, false, MSG.UPDATE_SUCCESS, course);
    } catch (error) {
      throw new ErrorExceptionWithResponse(
        error.status || StatusCodes.BAD_REQUEST,
        true,
  ...
  async getCourse(params, user) {
    try {
      ...
      return successResponse(StatusCodes.OK, false, MSG.FOUND_SUCCESS, course);
    } catch (error) {
      throw new ErrorExceptionWithResponse(
        error.status || StatusCodes.BAD_REQUEST,
        true,
  ...
  async UploadCourseCsv(user, file) {
    try {
      ...
      return successResponse(StatusCodes.OK, false, MSG.CREATE_SUCCESS);
    } catch (error) {
      throw new ErrorExceptionWithResponse(
        error.status || StatusCodes.BAD_REQUEST,
        true,
